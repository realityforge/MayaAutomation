...
def export_files():
    ...
    substance_painter_dir = require_json_value(model_repository_filename, model_repository_data,
                                               "Substance Painter Asset Directory", "substance_painter", "dir")
    unreal_export_dir = require_json_value(model_repository_filename, model_repository_data, "Unreal Import Directory",
                                           "unreal", "dir")
    ...
    if select_if_present(scene_short_name):
        ...
        raise Exception(
            "Unable to identify object to export for import into Unreal. No object named " + scene_short_name)
...
def load_json_data(filename):
    ...
    with open(qualified_filename, "r") as file:
        try:
            ...
            raise Exception(
                "Unable to load json from filename '" + filename + "' resolved to '" + qualified_filename + "' due to " + str(
                    e))
...
def require_json_value(filename, data, label, primary_key, secondary_key):
    ...
    if None != data[primary_key] and None != data[primary_key][secondary_key]:
        ...
        raise Exception(
            "Unable to extract " + label + " from configuration file " + filename + ". No json property named " + primary_key + "." + secondary_key + ".")
...
def polyCleanup(allMeshes, selectOnly, historyOn=False, quads=False, nsided=True, concave=True, holed=True,
    ...
    _args = '"{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}"'.format(
        _allMeshes, _selectOnly, _historyOn, _quads, _nsided, _concave, _holed, _nonplanar, _zeroGeom, zeroGeomTol,
        _zeroEdge, zeroEdgeTol, _zeroMap, zeroMapTol, _sharedUVs, _nonmanifold, _lamina, _invalidComponents)
